...
export class AuthenticationService {
  ...
  createUser(data: any) {
    this.httpClient.post('http://localhost:5000/api/users', data).subscribe(
      (result) => {
        console.log(result);
  ...
  autoAuthUser() {
    ...
    if (authInformation) {
      const now = new Date();
      const expiresIn =
        authInformation.expiretionDate.getTime() - now.getTime();
      if (expiresIn > 0) {
  ...
  login(user: { email: string; password: string }) {
    this.httpClient
      ...
      .subscribe(
        (result: any) => {
          ...
          if (result.token) {
            ...
            const expirationDate = new Date(
              date.getTime() + expiresInDuration * 3600
            );
        ...
        (err) => {
          this.toaster.error('There was problem in login.Please try again!');
        }
      );
  }
